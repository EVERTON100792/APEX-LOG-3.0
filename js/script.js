...
function saveFreightConfig() {
    ...
    // Recalcula fretes de todas as cargas ativas se tiverem distÃ¢ncia
    recalcAllFreights();
...
function calculateFreightValue(vehicleType, distanceKm) {
    ...
    if (vehicleType === 'fiorino') {
        if (distanceKm <= config.limit) {
            ...
            // InterpretaÃ§Ã£o: O valor TOTAL Ã© (distÃ¢ncia * taxa) ou (fixo + excedente)?
            ...
            // Mas se 301km * 1.80 = 541, e 300km = 225. Ã‰ um salto grande?
            // 300 * 1.80 = 540.
            // EntÃ£o 225 Ã© um valor MÃNIMO garantido ou tabela especÃ­fica?
            // O prompt diz: "se passar de 300 ganham 1,80 o rodado, se for ate 300 ganham 225 fixo".
            // Isso implica logicamente: DistÃ¢ncia <= 300 -> 225. DistÃ¢ncia > 300 -> DistÃ¢ncia * 1.80.
            freight = distanceKm * config.rate;
        }
    } else {
        // Vans, 3/4, Toco: "se passar de X ganham Y o rodado".
        // Mesma lÃ³gica. Se <= X, ganham "Tabela".
        if (distanceKm <= config.limit) {
...
function updateLoadFreightDisplay(loadId, distanceKm) {
    ...
    // Atualiza a distÃ¢ncia no objeto load se fornecida
    if (distanceKm) {
        load.distanceKm = parseFloat(distanceKm);
        saveStateToLocalStorage();
    }

    // Se nÃ£o tem distÃ¢ncia salva, nÃ£o calcula
    ...
    if (freightEl) {
        if (freightValue > 0) {
            ...
            // Se valor for 0 (ex: tabela nÃ£o definida), mostra aviso?
